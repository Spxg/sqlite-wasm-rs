...
**
** Original Source: https://sqlite.org/src/file?name=ext/misc/uuid.c&ci=tip
*/
...
/*
** [sqlite-wasm-rs]
** The original source includes <ctype.h> here. We have commented it out because
** the minimal libc shim used in this project does not include the implementation
** for ctype functions (isxdigit), which causes "undefined symbol" errors at
** runtime in WASM. We use a local implementation (sqlite3Isxdigit) instead.
*/
/* #include <ctype.h> */
...
** The minimal musl shim used in this project does not include ctype implementation,
** which causes "undefined symbol" errors at runtime in WASM.
*/
...
static int sqlite3UuidStrToBlob(
  ...
  for(i=0; i<16; i++){
    if( zStr[0]=='-' ) zStr++;
    if( sqlite3Isxdigit(zStr[0]) && sqlite3Isxdigit(zStr[1]) ){
...
int sqlite3_uuid_init(
  ...
  rc = sqlite3_create_function_v2(db, "uuid", 0, SQLITE_UTF8|SQLITE_INNOCUOUS, 0,
                               sqlite3UuidFunc, 0, 0, 0);
  if( rc==SQLITE_OK ){
    rc = sqlite3_create_function_v2(db, "uuid_str", 1, 
                       SQLITE_UTF8|SQLITE_INNOCUOUS|SQLITE_DETERMINISTIC,
                       0, sqlite3UuidStrFunc, 0, 0, 0);
  }
  if( rc==SQLITE_OK ){
    rc = sqlite3_create_function_v2(db, "uuid_blob", 1,
                       SQLITE_UTF8|SQLITE_INNOCUOUS|SQLITE_DETERMINISTIC,
                       0, sqlite3UuidBlobFunc, 0, 0, 0);
  }
  return rc;
}