...
pub unsafe fn sqlite3_free(arg1: *mut ::std::os::raw::c_void) {
    ...
    if ALLOCATED.lock().remove(&Ptr(arg1)).is_none() {
...
pub unsafe fn sqlite3_create_function_v2(
    ...
    let xFunc = xFunc.map(|f| {
        Closure::new(
            move |ctx: *mut sqlite3_context, values: Vec<JsValue>| -> JsValue {
                ...
                for value in values {
                    ...
                    SQLITE3_VALUES.lock().insert(Ptr(ptr as _), JsValueX(value));
                }
                f(
                    ctx,
                ...
                CONTEXT_ALLOCATED.lock().remove(&Ptr(ctx as _));
                // remove allocted sqlite3_values

                let mut locked1 = SQLITE3_VALUES.lock();
                let ret = locked1
                    .remove(&Ptr(ctx as _))
                    .map_or_else(|| JsValue::null(), |x| x.0);

                let mut locked2 = SQLITE3_VALUES_ALLOCATED.lock();
    ...
    let xStep = xStep.map(|f| {
        Closure::new(move |ctx: *mut sqlite3_context, values: Vec<JsValue>| {
            let mut sqlite3_values = Vec::with_capacity(values.len());
            for value in values {
                let ptr = wasm.alloc_ptr(1, true);
                sqlite3_values.push(ptr as *mut sqlite3_value);
...
pub unsafe fn sqlite3_bind_blob(
    ...
    if arg3.is_null() {
        ...
        let wasm_ptr = wasm.alloc(n as u32);
        wasm.poke(slice, wasm_ptr);
...
pub unsafe fn sqlite3_bind_text(
    ...
    if arg3.is_null() {
        ...
        let ptr = wasm.alloc(arg4 as u32);
        let slice = std::slice::from_raw_parts(arg3 as *const u8, arg4 as usize);
        wasm.poke(slice, ptr);
        let ret = capi.sqlite3_bind_text(arg1, arg2, ptr as _, arg4, dtor);
...
pub unsafe fn sqlite3_value_free(arg1: *mut sqlite3_value) {
    SQLITE3_VALUES_ALLOCATED.lock().remove(&Ptr(arg1 as _));
    if SQLITE3_VALUES.lock().remove(&Ptr(arg1 as _)).is_none() {
        sqlite().capi().sqlite3_value_free(arg1);
    }
}

pub unsafe fn sqlite3_value_bytes(arg1: *mut sqlite3_value) -> ::std::os::raw::c_int {
    if let Some(val) = SQLITE3_VALUES.lock().get(&Ptr(arg1 as _)) {
        let js = &val.0;
...
pub unsafe fn sqlite3_value_text(arg1: *mut sqlite3_value) -> *const ::std::os::raw::c_uchar {
    let ret = if let Some(val) = SQLITE3_VALUES.lock().get(&Ptr(arg1 as _)) {
        val.0.as_string().unwrap()
    } else {
        sqlite().capi().sqlite3_value_text(arg1)
    };
    ...
    SQLITE3_VALUES_ALLOCATED
        .lock()
        .insert(Ptr(arg1 as _), AllocatedT::CStr(ptr));
    ptr as _
}
...
pub unsafe fn sqlite3_value_blob(arg1: *mut sqlite3_value) -> *const ::std::os::raw::c_void {
    ...
    let mut data = if let Some(val) = SQLITE3_VALUES.lock().get(&Ptr(arg1 as _)) {
...
pub unsafe fn sqlite3_value_dup(arg1: *const sqlite3_value) -> *mut sqlite3_value {
    ...
    let mut locked = SQLITE3_VALUES.lock();
...
pub unsafe fn sqlite3_finalize(pStmt: *mut sqlite3_stmt) -> ::std::os::raw::c_int {
    STMT_ALLOCATED.lock().remove(&Ptr(pStmt as _));

    let mut locked = SQLITE3_VALUES_ALLOCATED.lock();
...
pub unsafe fn sqlite3_step(arg1: *mut sqlite3_stmt) -> ::std::os::raw::c_int {
    STMT_ALLOCATED.lock().remove(&Ptr(arg1 as _));
...
pub unsafe fn sqlite3_errmsg(arg1: *mut sqlite3) -> *const ::std::os::raw::c_char {
    ...
    ERR_MSG.lock().replace(AllocatedT::CStr(raw));
    raw
}
...
pub unsafe fn sqlite3_aggregate_context(
    ...
    let mut locked = CONTEXT_ALLOCATED.lock();
