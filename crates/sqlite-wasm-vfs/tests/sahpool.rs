...
use rsqlite_vfs::{
    transfer::{DbTransfer, ImportDbError, TransferError},
    VfsFilesManager,
};
use sqlite_wasm_rs::*;
...
// Exercise short reads, split headers and Interrupted without buffering the image.
struct SmallReads<R>(R, bool);
...
#[wasm_bindgen_test]
async fn test_reader_import_export_round_trip() {
    let util = pool("reader-round-trip", 3).await;
    let db = Db::open("reader-round-trip", "source.db");
    db.exec(c"CREATE TABLE t(n, data); INSERT INTO t VALUES(42, zeroblob(16384));");
    drop(db);

    let reader = util.export_db_reader("source.db").unwrap();
    util.import_db_from_reader("copy.db", SmallReads(reader, false))
        .unwrap();
    assert_eq!(
        util.export_db("source.db").unwrap(),
        util.export_db("copy.db").unwrap()
    );

    util.pause().unwrap();
    util.resume().await.unwrap();
    let db = Db::open("reader-round-trip", "copy.db");
    assert_eq!(db.scalar(c"SELECT n FROM t"), 42);
    assert_eq!(db.scalar(c"SELECT length(data) FROM t"), 16384);
    assert_eq!(
        db.scalar(c"SELECT count(*) FROM pragma_integrity_check WHERE integrity_check != 'ok'"),
...
#[wasm_bindgen_test]
async fn test_reader_import_validation_and_cleanup() {
    let util = pool("reader-validation", 1).await;
    let mut bytes = [0; 512];
    bytes[..16].copy_from_slice(b"SQLite format 3\0");
    bytes[16..20].copy_from_slice(&[2, 0, 2, 2]);
    util.import_db_from_reader("checked", SmallReads(bytes.as_slice(), false))
        .unwrap();
    let mut expected = bytes;
    expected[18..20].copy_from_slice(&[1, 1]);
    assert_eq!(util.export_db("checked").unwrap(), expected);
    assert!(util.remove("checked").unwrap());

    for (input, expected_error) in [(&bytes[..0], 0), (&bytes[..511], 0), (&[0; 512][..], 1)] {
        let error = util.import_db_from_reader("invalid", input).unwrap_err();
        assert!(matches!(
            (error, expected_error),
            (
                OpfsSAHError::Transfer(TransferError::ImportDb(ImportDbError::InvalidDbSize)),
                0
            ) | (
                OpfsSAHError::Transfer(TransferError::ImportDb(ImportDbError::InvalidHeader)),
                1
            )
        ));
        assert!(util.is_empty().unwrap());
    ...
    for (page_size, alignment_error) in [(513u16, false), (1024, true)] {
        let mut invalid = bytes;
        invalid[16..18].copy_from_slice(&page_size.to_be_bytes());
        let error = util
            .import_db_from_reader("invalid", invalid.as_slice())
            .unwrap_err();
        assert!(matches!(
            ...
            (
                ...
                OpfsSAHError::Transfer(TransferError::ImportDb(ImportDbError::InvalidPageSize)),
                false
            )
        ));
        assert!(util.is_empty().unwrap());
    ...
    for input in [bytes.as_slice(), b"encrypted\0bytes", b""] {
        util.import_db_from_reader_unchecked("opaque", SmallReads(input, false))
            .unwrap();
        assert_eq!(util.export_db("opaque").unwrap(), input);
        assert!(util.remove("opaque").unwrap());
    }

    for cleanup_fails in [false, true] {
        let fault = cleanup_fails.then(|| Restore(fail_next("flush", 1, "UnknownError")));
        let error = util
            .import_db_from_reader("failed", bytes.as_slice().chain(FailedRead))
            .unwrap_err();
        drop(fault);
        let error = if cleanup_fails {
            let OpfsSAHError::Cleanup { error, cleanup } = error else {
                panic!("expected input and cleanup errors");
            };
            assert!(matches!(
                *cleanup,
                OpfsSAHError::Opfs {
        ...
        assert!(
            matches!(error, OpfsSAHError::Reader(error) if error.kind() == io::ErrorKind::ConnectionReset)
        );
        assert!(util.is_empty().unwrap());
        assert_eq!(util.capacity(), 1);
    }
    ...
    assert!(matches!(
        util.import_db_from_reader("failed", bytes.as_slice()),
        Err(OpfsSAHError::Opfs {
    ...
    drop(fault);
    assert!(util.is_empty().unwrap());
    util.import_db_from_reader("reused", bytes.as_slice())
        .unwrap();
    assert_eq!(util.export_db("reused").unwrap(), expected);
    unsafe { util.uninstall().unwrap() };
}

#[wasm_bindgen_test]
async fn test_reader_guards_and_eof() {
    let util = pool("reader-guards", 2).await;
    util.import_db_unchecked("source", b"source bytes").unwrap();
    let mut reader = util.export_db_reader("source").unwrap();
    ...
    assert!(matches!(util.remove("source"), Err(OpfsSAHError::Busy)));
    assert!(matches!(
        Db::try_open("reader-guards", "source"),
        Err(SQLITE_CANTOPEN)
    ));
    assert!(matches!(
        util.import_db_from_reader_unchecked("source", b"replacement".as_slice()),
        Err(OpfsSAHError::FileExists(_))
    ));

    // Known-length imports can also coexist with a reader, without releasing it.
    util.import_db_unchecked("copy", b"copy bytes").unwrap();
    let fault = Restore(fail_next("read", 1, "UnknownError"));
    assert_eq!(
    ...
    assert!(matches!(util.pause(), Err(OpfsSAHError::Busy)));
    drop(reader);
    assert_eq!(util.export_db("copy").unwrap(), b"copy bytes");
    util.pause().unwrap();
    util.resume().await.unwrap();
    assert_eq!(util.export_db("source").unwrap(), b"source bytes");
    unsafe { util.uninstall().unwrap() };
}
