...
struct MemStore<C>(PhantomData<C>);

impl<C> VfsStore<MemFile, MemAppData> for MemStore<C>
where
    C: OsCallback,
{
...
struct MemIoMethods<C>(PhantomData<C>);

impl<C> SQLiteIoMethods for MemIoMethods<C>
where
    C: OsCallback,
{
    type File = MemFile;
    type AppData = MemAppData;
    type Store = MemStore<C>;
...
impl<C> SQLiteVfs<MemIoMethods<C>> for MemVfs<C>
where
    C: OsCallback,
{
    const VERSION: ::core::ffi::c_int = 1;
